...
void parallel_matrix_multiply(long **A, size_t num_rows_a, size_t num_cols_a,
                              long **B, size_t num_rows_b, size_t num_cols_b,
                              long **C) {
    // Mutex para garantizar exclusión mutua al acceder a la matriz resultado C
    std::mutex mtx;

    auto thread_function = [&](size_t start, size_t end) {
        for (size_t i = start; i < end; ++i) {
            for (size_t j = 0; j < num_cols_b; ++j) {
                ...
                // Se adquiere el lock antes de actualizar la celda de la matriz resultado C
                mtx.lock();
                C[i][j] = sum;
                // Se libera el lock después de actualizar la celda de la matriz resultado C
                mtx.unlock();
            }
        }
    };
    ...
    const size_t chunk_size = num_rows_a / num_threads;
